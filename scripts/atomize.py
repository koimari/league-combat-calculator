...
def atomize_champions(champions: dict) -> dict[str, list]:
    ...
    try:
        ...
        return {"_error": [{"atom_id": "champion_atomizer_unavailable", "behavior": "error",
                            "source": str(exc), "name": "champion atomizer", "values": [],
                            "units": [], "evidence": [str(exc)], "hash": "unavailable"}]}
    results = {}
...
def main(argv=None) -> int:
    ...
    parser.add_argument("domains", nargs="*", help="one of %s or 'all'" % "|".join(DOMAINS))
    ...
    if unknown:
        print(f"unknown domain(s): {sorted(unknown)}; use one of {DOMAINS} or 'all'", file=sys.stderr)
        return 2
    if not wanted:
        parser.print_help()
        return 2

    data = {key: json.loads(path.read_text(encoding="utf-8")) for key, path in _load_data().items()}
    out_root = Path(args.out)
    manifest = {}

    if "items" in wanted:
        objects = atomize_item_catalogue(data["items.json"])
        manifest["items"] = write_atoms(out_root / "items.json", domain="items", objects=objects)
    if "abilities" in wanted:
        objects = {
            name: atomize_abilities(name, champion)
            for name, champion in sorted(data["champions.json"].items())
        }
        manifest["abilities"] = write_atoms(out_root / "abilities.json", domain="abilities", objects=objects)
    if "runes" in wanted:
        objects = atomize_rune_catalogue(data["runes.json"])
        manifest["runes"] = write_atoms(out_root / "runes.json", domain="runes", objects=objects)
    if "economics" in wanted:
        objects = atomize_economics(data["economics-sourced.json"])
        manifest["economics"] = write_atoms(out_root / "economics.json", domain="economics", objects=objects)
    if "stats" in wanted:
        objects = {
            name: atomize_stats(champion)
            for name, champion in sorted(data["champions.json"].items())
        }
        manifest["stats"] = write_atoms(out_root / "stats.json", domain="stats", objects=objects)
    if "champions" in wanted:
        objects = atomize_champions(data["champions.json"])
        manifest["champions"] = write_atoms(out_root / "champions.json", domain="champions", objects=objects)

    (out_root / "manifest.json").write_text(
        json.dumps({"schema_version": 1, "domains": manifest}, indent=1, ensure_ascii=False)
    )
