"""The one walk over `item_support_effects`' damage-modifier packet sites.
...
def sites(kind: str = PACKET_KIND) -> list[Site]:
    """Every ``_packet`` construction site of one kind, in source order."""
    source = Path(item_support_effects.__file__).read_text(encoding="utf-8")
    found = []
    for node in ast.walk(ast.parse(source)):
        ...
        if _names_kind(keywords.get("kind"), kind):
            found.append(Site(node.lineno, keywords))
    return found
...
def declared(*names: str) -> dict[str, dict[str, Any]]:
    ...
    return {
        _literal_source(site): {name: _evaluate(site.keyword(name)) for name in names}
        for site in sites()
    }


def _evaluate(expression: ast.expr | None) -> Any:
...
def check() -> list[str]:
    ...
    for site in sites():
        if _literal_source(site) is None:
            findings.append(
                f"line {site.line} names no literal source; the registry "
