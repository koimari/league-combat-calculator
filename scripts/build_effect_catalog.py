...
def _text(item: dict[str, Any]) -> str:
    ...
    parts.extend(str(effect.get("effects") or "") for effect in item.get("passives", []) if isinstance(effect, dict))
...
def _effect(item: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = (
        ...
        ("damage_over_time", any(token in text for token in ("burn", "per second", "per tick", "over 3 seconds"))),
        ("damage_amp", any(token in text for token in ("bonus damage", "more damage", "damage taken"))),
        ...
        ("healing", bool(re.search(r"\bheal(?:ing)?\b|lifesteal|omnivamp|regenerat", text))),
    ...
    order: list[str] = []
    if any(token in text for token in ("damaging ability", "ability hits", "dealing damage", "attacks deal")):
    ...
    if "shield" in text or re.search(r"\bheal(?:ing)?\b|lifesteal|omnivamp|regenerat", text):
    ...
    return {
        ...
        "passives": [
            ...
            if isinstance(effect, dict) and (effect.get("name") or effect.get("effects"))
        ],
...
def build_catalog(source: Path, patch: str) -> dict[str, Any]:
    ...
    items = {_effect(item)["id"]: _effect(item) for item in raw.values() if isinstance(item, dict) and not item.get("removed")}
    return {
        "schema_version": 1,
        "patch": patch,
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=root / "static" / "effect-catalog.json")
