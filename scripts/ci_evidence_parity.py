...
a CI test" failure class.
    ...
    python scripts/ci_evidence_parity.py

`tests/test_ci_evidence_parity.py` drives the same scanner, one test per
discovered reference, and pins the extraction rules against fabricated source.
...
and are watched too; :data:`WATCHED_PREFIXES` is the one list. The remedy
for each occurrence is to force-track the evidence file, and this module is
the systemic guard so the next one is caught here instead of in CI.
...
remedies: track the file, or add one of the guard idioms above.
...
#: The scanner's own suite, whose fabricated sources name evidence paths in
...
def _iter_statement_containers(root: ast.AST):
    ...
    Narrower than ``ast.walk``, which also visits deep ``assert`` and
    ``Call`` expression trees: a docstring can only live in a statement
    list, and walking the rest is most of this scanner's runtime.
    """
    stack = [root]
    while stack:
        current = stack.pop()
...
def _raw_candidates(source: str) -> dict[str, int]:
    ...
    this finds at least one candidate in (see ``scan_text``), since the
...
def scan_text(rel_file: str, source: str, *, is_python: bool) -> list[Reference]:
...
def discover() -> tuple[list[Reference], dict[str, str]]:
    ...
    for path in sorted(TESTS_DIR.rglob("*")):
        ...
        texts[rel] = source
        references.extend(scan_text(rel, source, is_python=path.suffix == ".py"))
    return references, texts


def tracked_evidence_paths() -> frozenset[str]:
...
ALL_REFERENCES, _FILE_TEXT = discover()
TRACKED_PATHS = tracked_evidence_paths()


def has_guard(rel_file: str) -> bool:
    return bool(_GUARD_RE.search(_FILE_TEXT.get(rel_file, "")))


def unresolved() -> list[Reference]:
    """Every discovered reference that is neither tracked nor guarded."""
    return [
        reference
        for reference in ALL_REFERENCES
        if reference.path not in TRACKED_PATHS and not has_guard(reference.file)
    ]


def remedy(reference: Reference) -> str:
    """What to do about one unresolved reference, in full."""
    return (
        ...
        "`pytest.mark.skipif`). It passes locally and errors on CI. "
        "Remedy: EITHER force-track the evidence file (`git add -f "
        f"{reference.path}`) OR add one of the guard idioms above so the "
        "test skips instead of erroring when the local cache is absent."
    )
...
if __name__ == "__main__":
    raise SystemExit(
        report(
            [remedy(reference) for reference in unresolved()],
            f"OK: {len(ALL_REFERENCES)} evidence references, each tracked or guarded",
        )
    )
