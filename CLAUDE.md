...
- **cProfile shares lie about this engine, and `bench_optimize_build.py` cannot resolve a small win:** the fight engine's cost is spread across millions of one-line helpers, so the profiler's ~1 µs per call over-weights them roughly 3× (`behavior_rules` read 8% and was 4.7%). Take call *counts* from the profile and *shares* from a `timeit` best-of-7 against the whole evaluation. The search bench's own noise floor is ±3%, so anything under that needs the per-evaluation instrument, and a per-build memo measured in a loop over one build hides the miss the search pays on every evaluation. Budget table: `benchmarks.md`.
