...
python scripts/property_sweep.py      # Property gate: rules every fight obeys (a longer fight never deals less, more armour never deals more, a trace sums to its total, ...) over every champion bare and in a seeded random legal build (own CI job, 4 shards). A violation fails unless `ACKNOWLEDGED` names it with its reason
