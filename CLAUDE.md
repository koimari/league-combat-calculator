...
python scripts/prose_lint.py          # Python prose gate over src/ and scripts/: docstrings and comments hold current state, none longer than its body, no banner over an empty section, and under champions/ a 20-line module header and a 120-character assumption string. `pointer`, the one rule that also reaches tests/, fails on prose citing a phase, a decision id or a named unit of work unless the same line names a path that resolves. `unsourced_constant` reports under a ceiling tests/test_prose_lint.py holds, which may only fall. It reads no markdown; the plugin hook `comment_lint.lint_prose` is the markdown one
