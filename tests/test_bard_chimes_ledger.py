...
class TestSourceAndTypedValues:
    ...
    def test_wiki_prose_pins_the_numbers_and_boundaries(self):
        ...
        for note in (
            "Meep autos are min(autos, stock + fight_duration / recharge), "
            "the rest plain; one rotation uses stock only.",
            "Meep slow and the 15+ chime AoE/cone splash are not modeled: "
            "single target, and the splash misses the primary.",
        ):
...
class TestUnchangedBoundaries:
    ...
    def test_q_unchanged(self):
        ...
        assert abilities["Q"]["total_raw"] == pytest.approx(
            _resolve(
                "Q", "Magic Damage", 4, dict(parse_stats(_LEVEL), ability_power=100.0)
            )
        )
