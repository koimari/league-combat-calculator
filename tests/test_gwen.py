"""Reviewed crowd control for Gwen (MODULE_CC, wave 4B).

Needlework slows; Skip 'n Slash only empowers attacks.
"""
...
class TestReviewedCrowdControl:
    """Skip 'n Slash only empowers attacks; Needlework's slow is undeclared.
    ...
    def test_module_cc_is_the_declaration_the_parser_wired(self):
        ...
        assert gwen.MODULE_CC == {"E": "none"}
        assert gwen.parse_abilities.cc_kinds == gwen.MODULE_CC

    def test_needleworks_slow_is_in_the_text_the_module_left_undeclared(self):
        """R's kind is real; declaring it costs a Q cast Q's own row cannot repay."""
        assert "slow" in _cc_slot_text("R")

    def test_control_free_slots_name_every_word_their_text_contains(self):
        for slot, expected in [["E", []]]:
            assert _cc_control_hits(slot) == list(expected), slot

    def test_every_reviewed_part_carries_its_kind(self):
        assert _cc_kinds() == {"E": ["none"]}

    def test_a_timed_fimbulwinter_fight_is_still_coarse_on_q(self):
        """Snip Snip! aggregates 'at least twice' snips over the cast time with no sourced per-snip cadence — so Q reaches the ledger unreviewed and the kit stays coarse."""
