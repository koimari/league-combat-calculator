...
def test_an_omitted_optional_int_is_none_rather_than_a_default():
    ...
    assert request_optional_int({}, "gold_budget", maximum=30_000) is None
    assert (
        request_optional_int({"gold_budget": None}, "gold_budget", maximum=30_000)
        is None
    )
    assert (
        request_optional_int({"gold_budget": ""}, "gold_budget", maximum=30_000) is None
    )
...
def test_a_supplied_optional_int_reads_under_the_shared_integer_policy():
    ...
    assert request_optional_int(data, "gold_budget", maximum=30_000) == 2500
    assert (
        request_optional_int({"gold_budget": 2500}, "gold_budget", maximum=30_000)
        == 2500
    )

    with pytest.raises(ValueError, match="gold_budget must be an integer"):
        request_optional_int({"gold_budget": True}, "gold_budget", maximum=30_000)
    with pytest.raises(ValueError, match="gold_budget must be an integer"):
        request_optional_int({"gold_budget": "2500.0"}, "gold_budget", maximum=30_000)
    with pytest.raises(ValueError, match="gold_budget must be between 1 and 30000"):
        request_optional_int({"gold_budget": 0}, "gold_budget", maximum=30_000)
...
def test_zero_is_a_value_and_not_an_absent_field():
    ...
    with pytest.raises(ValueError, match="max_purchase_items must be between 1 and 7"):
        request_optional_int({"max_purchase_items": 0}, "max_purchase_items", maximum=7)
    assert (
        request_int(
