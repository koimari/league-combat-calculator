...
class TestQAvengerang:
    ...
    def test_q_is_physical_damage(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
        )
        assert abilities["Q"]["damage_type"] == "physical"

    def test_q_has_cooldown(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
        )
    ...
    def test_q_rank1_total_damage(self, akshan_data: dict) -> None:
        """Q rank 1 Total: 90 + 140% bonus AD."""
        stats = calculate_total_stats(akshan_data, 1, [])
        abilities = parse_abilities(
            akshan_data, 1, 0.0, champion_stats=stats,
        )
        q = abilities["Q"]
    ...
    def test_q_uses_total_not_single_pass(self, akshan_data: dict) -> None:
...
class TestWGoingRogue:
    ...
    def test_w_not_in_results(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 9, [])
        abilities = parse_abilities(
            akshan_data, 9, 0.0, champion_stats=stats,
        )
...
class TestEHeroicSwing:
    ...
    def test_e_is_physical_damage(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
    ...
    def test_e_has_cooldown(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
            champion_options={"e_shots": 5},
    ...
    def test_e_per_shot_rank1(self, akshan_data: dict) -> None:
    ...
    def test_e_total_scales_with_shots(self, akshan_data: dict) -> None:
        ...
        assert abs(
            six["E"]["total_raw"] - three["E"]["total_raw"] * 2
        ) < 0.1

    def test_e_zero_shots_excluded(self, akshan_data: dict) -> None:
...
class TestRComeuppance:
    ...
    def test_r_is_physical_damage(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 6, [])
        abilities = parse_abilities(
            akshan_data, 6, 0.0, champion_stats=stats,
        )
        assert abilities["R"]["damage_type"] == "physical"

    def test_r_has_cooldown(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 6, [])
        abilities = parse_abilities(
            akshan_data, 6, 0.0, champion_stats=stats,
        )
    ...
    def test_r_rank1_base_damage(self, akshan_data: dict) -> None:
    ...
    def test_r_has_crit_effectiveness(self, akshan_data: dict) -> None:
    ...
    def test_r_has_missing_hp_max_bonus(self, akshan_data: dict) -> None:
    ...
    def test_r_rank2_has_6_bullets(self, akshan_data: dict) -> None:
        """R rank 2: 6 bullets * (35 + 15% AD) min per bullet."""
        stats = calculate_total_stats(akshan_data, 11, [])
        abilities = parse_abilities(
            akshan_data, 11, 0.0, champion_stats=stats,
        )
        r = abilities["R"]
        ad = stats["attack_damage"]
...
class TestPassiveDoubleShot:
    ...
    def test_double_shot_in_results(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
        )
    ...
    def test_double_shot_has_ad_ratio(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
        )
    ...
    def test_double_shot_ratio_extraction(self, akshan_data: dict) -> None:
...
class TestPassiveDirtyFighting:
    ...
    def test_passive_in_results_with_procs(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
    ...
    def test_passive_is_magic_damage(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
            champion_options={"passive_procs": 3},
    ...
    def test_passive_has_proc_count_field(self, akshan_data: dict) -> None:
        stats = calculate_total_stats(akshan_data, 5, [])
        abilities = parse_abilities(
            akshan_data, 5, 0.0, champion_stats=stats,
...
class TestFightEngineIntegration:
    ...
    def test_double_shot_in_breakdown(self, akshan_data: dict) -> None:
    ...
    def test_passive_proc_in_breakdown(self, akshan_data: dict) -> None:
    ...
    def test_crit_effectiveness_amplifies_r(self, akshan_data: dict) -> None:
        """R damage should be higher with crit chance due to 30% effectiveness."""
        stats_no_crit = calculate_total_stats(akshan_data, 6, [])
        stats_no_crit["critical_strike_chance"] = 0.0

        stats_with_crit = dict(stats_no_crit)
        stats_with_crit["critical_strike_chance"] = 100.0

        abilities_no_crit = parse_abilities(
            akshan_data, 6, 0.0, champion_stats=stats_no_crit,
        )
        abilities_with_crit = parse_abilities(
            akshan_data, 6, 0.0, champion_stats=stats_with_crit,
        )

        # Use high target HP so missing HP scaling is negligible
    ...
    def test_r_missing_hp_scaling(self, akshan_data: dict) -> None:
        ...
        # Q and E deal damage before R → target has more missing HP
        full = parse_abilities(
            akshan_data, 11, 0.0, champion_stats=stats,
            champion_options={"passive_procs": 0, "e_shots": 5},
        )
        # Remove double_shot so only abilities are in play
        abilities_for_test = {
            k: v for k, v in full.items() if k != "passive_double_shot"
        }
    ...
    def test_r_crit_formula_matches_wiki(self, akshan_data: dict) -> None:
        """Verify R crit formula: 3% per 10% crit chance.

        At 75% crit chance (no IE, base crit_multiplier = 2.0):
        crit_amp = 1 + 0.3*0.75 + 0.3*0.0*0.75 = 1.225
        """
        stats = calculate_total_stats(akshan_data, 18, [])
        stats["critical_strike_chance"] = 75.0

        abilities = parse_abilities(
            akshan_data, 18, 0.0, champion_stats=stats,
        )
        r_base = abilities["R"]["physical_damage"]

        # Use very high target HP to minimize missing HP scaling effect.
        # No items → crit_multiplier = 2.0, bonus_crit = 0.0
        result = calculate_fight_damage(
            champion_stats=dict(stats),
        ...
        # crit_amp = 1 + 0.3*0.75 = 1.225 (no IE → bonus_crit = 0)
        expected = r_base * 1.225
        assert abs(r_damage - expected) / expected < 0.02

    def test_all_abilities_present(self, akshan_data: dict) -> None:
