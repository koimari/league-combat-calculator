...
class TestCapBehavior:
    def test_kernel_cap_noop_and_consume_at_cap(self):
        ...
        denied = state.apply_gain(sl.EventStamp(4.0, 4), kind="basic_ability_cast")
        assert state.stacks == 4
        assert denied[-1].kind == "gain_denied"
        assert denied[-1].detail["reason"] == "at_cap"
        consumed = state.consume(sl.EventStamp(5.0, 5))
        ...
        assert state.consume(sl.EventStamp(6.0, 6)) is None
...
class TestTimerAndCombatExtension:
    ...
    def test_kernel_combat_extension_freezes_ten_seconds(self):
        ...
        state.note_activity(sl.EventStamp(0.5, 5), kind="damage_dealt")
        state._materialize_expiries(sl.EventStamp(1.0, 99))
...
class TestDotProcExclusion:
    def test_kernel_freeze_is_explicit_only(self):
        ...
        state._materialize_expiries(sl.EventStamp(9.9, 99))
        assert state.stacks == 1  # still frozen
        state._materialize_expiries(sl.EventStamp(10.0, 99))
