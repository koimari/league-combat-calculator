"""Focused full-entry and packet coverage for CP10.11."""
...
def test_cp10_11_modules_are_reviewed_and_have_full_entry_receipts():
...
def test_cp10_11_parses_every_passive_and_ability_slot(name):
    result = _parse(name)
    assert set(result) >= {"passive", "Q", "W", "E", "R"}
    assert all("parts" in row and "damage_type" in row for row in result.values())


def test_cp10_11_source_receipts_are_parent_plus_all_templates():
    for name in NEW_BATCH:
