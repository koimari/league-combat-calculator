...
def test_wiki_bis_profiles_cover_every_champion_and_slot():
    ...
    assert all(set(champion["abilities"]) == set(ABILITY_SLOTS) for champion in profiles["champions"].values())
...
def test_checked_in_bis_profiles_preserve_role_and_scaling_signals():
    ...
    assert "targetCurrentHp" in checked_in["champions"]["Dr. Mundo"]["abilities"]["Q"][0]["packets"][0]["ratios"]
    assert "bonusHp" in checked_in["champions"]["Dr. Mundo"]["abilities"]["E"][0]["packets"][0]["ratios"]
    assert checked_in["champions"]["Orianna"]["abilities"]["E"][0]["shields"][0]["base"]
    assert "ap" in checked_in["champions"]["Orianna"]["abilities"]["E"][0]["shields"][0]["ratios"]


def test_axword_meraki_reference_can_fill_unparsed_wiki_packets():
    auxiliary = ROOT.parent / "lol-strength-analysis" / "src" / "data" / "generated" / "merakiAbilityKits.ts"
