...
class TestReviewedCrowdControl:
    ...
    def test_mimicking_ethereal_chains_authors_no_kind(self):
    ...
    def test_the_chain_and_its_fracture_are_two_hits_the_cache_times(self):
        """E declares CC_PER_PART because its two hits answer differently:
    ...
    def test_the_reviewed_sigil_certifies_the_whole_fight(self):
        ...
        name, a 3.5-second window rather than a cadence, so the two stay
        one part.  Q controls nothing either way, and with that reviewed
        the kit leaves no ability event without a kind."""
        ...
        assert leblanc.MODULE_CC["Q"] == "none"
        assert cc_review.unreviewed_ability_slots("LeBlanc") == []
