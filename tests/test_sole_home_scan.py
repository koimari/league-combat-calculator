...
def test_the_forced_restatements_are_criterion_4s_counted_carve_out(scan) -> None:
    ...
    *coincidence* stands at one where the same capture put it, its event
    ...
    # which is exactly what the stale check above is for.  It then rose to two,
    # across two sources, when that capture's own allowlist landed a commit
    # later claiming the coupled leaf counter in order to supersede the claim
    # before it -- the fourth run of the pattern, and the first where the fall
    # and the rise sit in two commits rather than one, which is why the scan
    # reported the arriving site instead of nothing at all.
