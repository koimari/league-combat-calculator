...
class TestReviewedCrowdControl:
    """The rocket only explodes; W's slow and E's root stay unreadable.
    ...
    def test_module_cc_is_the_declaration_the_parser_wired(self):
        ...
        assert jinx.MODULE_CC == {"R": "none"}
        assert jinx.parse_abilities.cc_kinds == jinx.MODULE_CC

    def test_the_undeclared_slots_control_is_in_the_text_all_the_same(self):
        """W's and E's kinds are real; only their route to the ledger is not."""
        assert "slow" in _cc_slot_text("W")
        assert "root" in _cc_slot_text("E")
    ...
    def test_every_reviewed_part_carries_its_kind(self):
        assert _cc_kinds() == {"R": ["none"]}

    def test_a_timed_fimbulwinter_fight_is_still_coarse_on_w_and_e(self):
        """Neither packet authors an event a marker could ride."""
