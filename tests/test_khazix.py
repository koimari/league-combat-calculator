"""Kha'Zix — the reviewed crowd control its kit declares (MODULE_CC).
...
# Control words the reviewed-absent slots mention for another reason:
# "Taste Their Fear" is the Q ability's own name (and Q is castable during
# E's leap), and W's only slow belongs to the Evolved Spike Racks bonus,
# an evolution this module does not price.
UNCONTROLLED_MENTIONS = {"Q": ["fear"], "W": ["slow"], "E": ["fear"]}
...
class TestReviewedCrowdControl:
    ...
    def test_every_ability_event_carries_the_review(self, cached):
        ...
        parsed = khazix.parse_abilities(cached, 18, 100.0)
        for slot, kind in khazix.MODULE_CC.items():
