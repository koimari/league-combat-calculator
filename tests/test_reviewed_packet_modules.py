...
def test_every_generated_champion_has_a_reviewed_dedicated_module():
    asset = _asset()
    assert set(_GENERIC_CHAMPION_MODULES) <= set(asset["champions"])
    assert len(_GENERIC_CHAMPION_MODULES) == 126
    for champion, module_name in _GENERIC_CHAMPION_MODULES.items():
        ...
        assert module.REVIEW_STATUS == "reviewed_packet", champion
