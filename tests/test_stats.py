...
class TestCalculateTotalStats:
    ...
    def test_rabadons_multiplies_ap(
        self, ahri_data: dict, liandrys: dict, malignance: dict, rylais: dict, rabadons: dict
    ) -> None:
        from src.calculator.data_fetcher import get_item_by_name
    ...
    def test_bandlepipes_fanfare_attack_speed(self, ahri_data: dict) -> None:
        ...
        assert abs(stats_with["attack_speed"] - stats_without["attack_speed"] - expected_diff) < 0.01
    ...
    def test_hexplate_overdrive_attack_speed(self, ahri_data: dict) -> None:
        ...
        stats_without = calculate_total_stats(ahri_data, 18, [])
        as_ratio = ahri_data["stats"].get("attackSpeedRatio", {}).get(
            "flat", ahri_data["stats"]["attackSpeed"]["flat"]
        )
...
class TestStatsUsesItemEffectsRegistry:
    ...
    def test_rabadons_reads_from_registry(self, ahri_data: dict, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_seraphs_reads_from_registry(self, ahri_data: dict, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_muramana_reads_from_registry(self, ahri_data: dict, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestNewItemStats:
    ...
    def test_flowing_water_reads_from_registry(
        ...
        monkeypatch.setitem(item_effects.ITEM_EFFECTS, "Staff of Flowing Water", patched)
        stats = calculate_total_stats(ahri_data, 18, [staff])
    ...
    def test_statikk_shiv_parsed_values(self) -> None:
        """Parser extracts base=60, empowered_auto_count=3, damage_type=magic."""
        from src.calculator.passive_parser import parse_item_effect
        from src.calculator.data_fetcher import fetch_item_data
        items = fetch_item_data()
        parsed = parse_item_effect("Statikk Shiv", items)
        assert parsed["base"] == 60.0
        assert parsed["empowered_auto_count"] == 3
        assert parsed["damage_type"] == "magic"
