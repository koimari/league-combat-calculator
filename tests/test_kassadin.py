"""Kassadin — the reviewed crowd control its kit declares (MODULE_CC).
...
QUOTED = {"E": "slows them for 1 second"}
...
class TestReviewedCrowdControl:
    ...
    def test_reviewed_absences_read_the_whole_slot(self, cached):
        ...
        for slot, kind in kassadin.MODULE_CC.items():
            if kind != "none":
                continue
            hits = [
                word
    ...
    def test_every_ability_event_carries_the_review(self, cached):
        ...
        parsed = kassadin.parse_abilities(cached, 18, 100.0)
        for slot, kind in kassadin.MODULE_CC.items():
