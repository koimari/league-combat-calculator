...
class TestBladeRider:
    ...
    def test_the_rider_reaches_the_fight_on_both_channels(self):
        """Level 18, no items, timed: eight swings, Wild Rush's window inside."""
        ...
        assert row["count"] == on["breakdown"]["auto_attacks"]["count"] == 8
        # Each proc reads the target's decayed health, so they escalate.
        assert row["total_damage"] == pytest.approx(151.6, abs=0.05)
        ...
        assert on["total_damage"] == pytest.approx(1566.9, abs=0.05)
