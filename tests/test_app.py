...
def test_config_exposes_the_champion_review_boundary():
    ...
    assert engine["reviewed_count"] == 171
    assert engine["generated_count"] == 2
    assert engine["unreviewed_count"] == 2
...
class TestIconUrlsAreHttps:
    ...
    def test_champion_api_exposes_all_ingested_and_reviewed_slots(self):
        ...
        assert sum(champion["verified"] for champion in champions) == 171
        by_name = {champion["name"]: champion for champion in champions}
        assert by_name["Aatrox"]["engine_registration"] == "reviewed_module"
        assert by_name["Zoe"]["engine_registration"] == "generated_packet"
...
class TestChampionVerifiedFlags:
    ...
    def test_unverified_champions_expose_specific_fail_closed_reasons(self):
        ...
        assert by_name["Zoe"]["availability"]["ready"] is False
        assert by_name["Zoe"]["availability"]["verification"] == "blocked"
        assert by_name["Zoe"]["availability"]["blockers"]
        assert by_name["Zoe"]["engine_registration"] == "generated_packet"
        assert by_name["Zoe"]["patch_last_changed"]
