...
def test_config_exposes_the_champion_review_boundary():
    ...
    assert engine["reviewed_count"] == 101
    assert engine["generated_count"] == 72
    assert engine["unreviewed_count"] == 72
...
class TestIconUrlsAreHttps:
    ...
    def test_champion_api_exposes_all_ingested_and_reviewed_slots(self):
        ...
        assert sum(champion["verified"] for champion in champions) == 101
