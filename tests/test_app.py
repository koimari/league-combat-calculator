...
def test_config_exposes_the_champion_review_boundary():
    ...
    assert engine["reviewed_count"] == 111
    assert engine["generated_count"] == 62
    assert engine["unreviewed_count"] == 62
...
class TestIconUrlsAreHttps:
    ...
    def test_champion_api_exposes_all_ingested_and_reviewed_slots(self):
        ...
        assert sum(champion["verified"] for champion in champions) == 111
