...
class TestFightIntegration:
    ...
    def test_cannon_shred_spares_the_stream_swing_that_delivers_it(
        self, jayce_data
    ) -> None:
        """With an auto stream R rides a stream swing, which the auto row
        still holds when the shred windows re-price it: that swing meets
        the UNSHREDDED 100 armor too, so R's row is the same 125."""
        stats = dict(STATS_250_AD)
        abilities = _parse(jayce_data, stats=stats)
        result = calculate_fight_damage(
            stats,
...
class TestHyperChargeBurstAttackSpeed:
    ...
    def test_the_auto_row_keeps_every_swing_no_cast_claimed(
        self, jayce_data, monkeypatch
    ) -> None:
        """W's burst impacts and R's ride leave the auto row at their own
        times and prices, and every ordinary swing stays.

        Against 100 armor R's shred sets swings apart, so moving any swing
        but the claimed one would re-price both rows.
        """
        seen: dict[str, Any] = {}
        reattribute = damage._reattribute_empowered_swings

        def spy(state, cast_events):
            seen["stream"] = {
                event["time"]: event["damage"]
                for event in state.breakdown["auto_attacks"]["damage_events"]
            }
            seen["claims"] = {
                "W": state.burst_swings.by_ability["W"],
                "R": state.empowered_ride_times["R"],
            }
            seen["own"] = {
                key: state.breakdown[key]["total_damage"] for key in seen["claims"]
            }
            reattribute(state, cast_events)

        monkeypatch.setattr(damage, "_reattribute_empowered_swings", spy)
        stats = dict(STATS_250_AD, critical_strike_chance=0.0)
        result = calculate_fight_damage(
            stats,
            _parse(jayce_data, stats=stats),
            [],
            FightConfig(
                target_health=2500,
                target_armor=100,
        ...
        breakdown = result["breakdown"]
        claimed = {time for times in seen["claims"].values() for time in times}
        kept = {
            event["time"]: event["damage"]
            for event in breakdown["auto_attacks"]["damage_events"]
        }
        assert kept == {
            time: dealt for time, dealt in seen["stream"].items() if time not in claimed
        }
        for key, times in seen["claims"].items():
            assert breakdown[key]["total_damage"] == pytest.approx(
                seen["own"][key] + sum(seen["stream"][time] for time in times)
            )
