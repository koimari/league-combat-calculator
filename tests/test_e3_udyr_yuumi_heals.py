...
def _fight(champion: str, ranks: dict, role: str) -> dict:
    ...
    payload = {
        "champion": champion, "level": 18, "items": [],
        "role": role,
        "fight_mode": "time_based", "fight_duration": 10, "include_auto_attacks": True,
        "enemies": [{"champion": "Ahri", "level": 18, "items": [], "role": "mid",
...
def test_udyr_w_iron_mantle_heals_sixteen_ticks():
    ...
    heals = [e for e in combat.get("healing_events", []) if e.get("attacker") == "main" and e["source"] == "Iron Mantle"]
...
def test_yuumi_r_final_chapter_heals_five_waves():
    ...
    heals = [e for e in combat.get("healing_events", []) if e.get("attacker") == "main" and e["source"] == "Final Chapter"]
