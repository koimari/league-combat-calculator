...
- **Four concurrent full `pytest -n auto` runs take this machine out of memory.**
  ...
  worker runs only the files it touched, one pytest at a time, no `-n`, and passes
  `--jobs=4` to pylint.
- **A test that mutates shared state races the CI's `pytest -n auto` workers, and
  ...
  are parameters (`zero_policy_frontier(root=...)`). A gate needing a clean world
  `monkeypatch`es every member of its table, derived from the table, not a
  hand-listed subset. Never `.clear()` a process-wide cache; `cold_memo` in
  `tests/conftest.py` lends a cold one.
...
- **A full-suite run is untrustworthy while another process edits `src/`.** A test
  ...
  stops, or against a `git archive HEAD` copy, the one safe way to read the branch
...
- **pylint cannot infer any decorator that returns a closure**, so calling a
  decorated parser by name raises E1120 against the body's own signature, and only
  a `signature-mutators` config this tree lacks would fix it. A parser
...
- **A tracked-data guard is armed by its own test, and by any bare glob.**
  `tracked_data_lint.Readers.in_tree` harvests `tests/`, so a receipt name spelled
  in a fixture makes the guard cover it: generate absent names with `uuid4`. A
  glob counts as a reader only where it pins past the suffix (`names_a_family`), a
  docstring mention not at all. Read tracked files with `git ls-files`, never
  `rglob`, which races a scratch file under xdist. Deleting a tracked receipt's
  last reader turns the gate red with `orphan: <path>`.
- **A codemod over `tests/test_*.py` eats the file you just wrote the new tests
  into.** Refuse the destination by name, and stage the hand-written change first
  so `git checkout -- tests/` restores it. A codemod walking its own home wrote an
  import of itself into itself.
...
- **A pytest `-p` plugin proves what collection would otherwise hide.** It loads
  before collection imports the test module, so it can wrap the callee and digest
  every result, or stub an absent resource's probe. Filter `at 0x[0-9A-Fa-f]+` out
  of any digest: closure reprs differ run to run.
...
- **A guard reached through a requested fixture is reached.**
  `scripts/resource_markers.py` follows both edges; a call-graph-only scan left 14
  node-guarded tests unmarked. Its other silent pass is
  `any(path.glob(pattern) for path in dirs)`, true because a generator is truthy.
- **A `_today` or `_is_still_tracked` test name is not evidence that the body pins
  a defect, and a docstring claiming singularity is not a gate.** Four such tests
  assert the completed behaviour, and one helper claiming the tree's only AST walk
  was one of three. Read the body, then rename or delete.
...
- **A reference count cannot tell slop from a load-bearing symbol, and a re-grep
  ...
  so check with an AST pass over every `ImportFrom` under `src/`, `tests/` and
  `scripts/` against the names the diff removes. A revert switch holding one
  value, a Protocol unrelated dataclasses satisfy, a disambiguating alias and a
  derived index whose one reader is a gate all read as dead: count the reason.
...
- **Two zeros move the coupled golden with no number changing.**
  ...
  different leaf sets: keep the membership filter and the value read separate, as
  `tests/test_item_effects.py` pins. `public_response` skips a row with no damage,
  no amount and no `detail`, so a `detail` on a zero row makes it appear.
- **Two key orders are numeric, one is not.** Compiled slot order is Q,W,E,R,P
  ...
  entry's own key order is replayed nowhere.
...
- **A field read with a literal default is the rule-5 failure shape, inside a
  ...
  Read it through the stamp or refuse.
...
- **`golden_snapshot.py capture` refuses while `src/` differs from HEAD**, because
  the recorded `src_tree_sha` would name a tree the capture did not read. A
  behaviour fix is two commits: the src change, then the re-capture.
- **`docs/receipts/campaign-stages.json` outlives counter 4's deferrals**, because
  `creditor_stage` still publishes `retires_at` on every unserved-lane row.
  Deleting a table because one of its two readers left is how a receipt field goes
  unreadable.
- **Three gates pin a name, so moving what they name turns them red.**
  ...
  symbol per module: drop the row and regenerate with `--write`.
  `docs/receipts/er5-tail-triage.json` counts `x.get(key, literal)` sites under
  `src/`: regenerate with `python scripts/tail_site_triage.py write`.
...
- **Crit rolls are random unless `deterministic` is set.**
  ...
  It is not a drop-in for a `random.random` patch forcing a crit COUNT: the
  deterministic branch blends crit and non-crit at expected value and sets
  `natural_crit` False, so `num_crits` is 0 and a crit-only rider publishes no row.
- **The walk mutates the `actions` list its caller passes.** `run_survival_walk`
  ...
  every walk-authored packet with no error.
...
- **A section-numbered citation into an append-only log is a second home for every
  number it quotes, and it drifts silently.** One such section was stale in two of
  its six Eclipse number groups against `item_effects.py`, with no way for the
  test naming it as source of truth to notice. Cite the accessor key.
...
- **Which writer produced a file decides its newlines on this CRLF tree.**
  ...
  `b.replace(b"\r\n", b"").count(b"\n") == 0` after black. `Path.write_text`
  re-emits `\n` as `\r\n`, matching the neighbours; a rewriter opening with
  `newline=""` and the Write tool both write LF. Commit that and let `autocrlf`
  normalize: hand-converting leaves a spurious second diff.
- **A bad path in a pytest argument list is silent.** A written file list reaches
  ...
  `pytest -n 4` prints "no tests ran" and names no file.
- **A parameters-to-record codemod turns `del <param>` into `del ctx.<field>`**,
  ...
  black and pylint. Where `del` only silenced `unused-argument` the statement goes.
  Scan for `ast.Delete` over every rewritten function.
- **Codemod bites around `ast` and black.** `ast` ends a parenthesized implicit
  ...
  `from .x import a, b, c` on one line. Run black before comparing lint, and count
...
- **The Bash tool mangles a backslash inside a quoted heredoc.** `"\\\n"` in a
  ...
  `chr(10)`, and assert the match count before writing.
- **Two cached-data quirks the vendored parser owns.** A Windows filename cannot
  ...
  from cache filenames, and the local copy patches the `nvalues=None` crash on
  Heimerdinger, Sona, Karma and Nidalee.
...
- **Parallel sessions share one `.git`.** A worktree forks from `main`'s tip, not
  the session's branch, so verify `git merge-base` against the intended base.
  `git checkout -- <dir>` in one worktree discards another session's uncommitted
  edits there, and `git stash` is one stack across all worktrees: detach onto the
  base commit instead. Take a branch another worktree holds with
  `git checkout --ignore-other-worktrees`. Here `git rm skill.md` also removes
  `SKILL.md`.
...
- **`data/atoms/manifest.json` `source_ref` digests hash LF bytes.** The corpus is
  ...
  tracked, so the champions domain resolves locally, but regenerate `data/atoms`
  on Linux only.
- **The simply-elegant hooks resolve `pyproject.toml` per-file rules against the
  ...
  exemptions as errors, and the file-length hook's `git show HEAD:<path>` baseline
  reports growth even for an edit that shrinks the file. Verify a worktree hit at
  the repo root rather than adding a marker.
- **A Bash task that times out and is moved to the background re-runs its whole
  ...
  an exact-string script deleted right after. A regex over a docblock with `re.S`
  is the same trap in one step: an optional group swallows everything from the
  file's first docblock to the target.
- **Three ruff autofixes bite on this tree.** F401 strips an accidental re-export
  ...
  C414's `tuple(list(x))` to `tuple(x)` returns the same object when `x` is a
  tuple. PLW0108's lambda removal hoists a forward reference into a `NameError`.
  Import-check every champion module after a fix pass.
...
- **Sightline counts move in both directions after a record or leaf extraction,
  ...
  `slot_extract.extract_named`. #11 has a size floor, so
  `ctx.damage_events, ctx.cast_timeline` crossed it where the bare names did not,
  and #55 reports a six-positional signature once its siblings stop sharing the
  shape. Dissolve them on the record rather than baselining them.
- **Sightline #11 normalizes names and literals**, so a wrapper differing only in
  its regex and message is still a clone, and a one-statement `main` is big enough
  to count. Calling one shared helper does not dissolve a clone group; a record or
  a `partial` binding does. The plugin's ruff wants `x.get("k") or ()` (FURB110)
  where the rule-5 lint reads an or-default: bind first, then
...
- **A codemod that writes files with Python bypasses the per-edit hook entirely**,
  ...
  branch base separates a new finding from the ones already blocking.
- **Two prose gates, one per language.** `comment_lint.lint_prose` reads every
  changed `.md` whole, banning em dashes, task markers and a fixed history phrase
  list outside fenced blocks and `prose-ok` lines, so one edit to a file carrying
  the debt clears that whole file; most tracked markdown outside the three root
  docs and the five skill files still carries em dashes. `scripts/prose_lint.py`
  compares a docstring against its function body span, so removing lines from a
  body pushes an unchanged docstring over its budget: move the explanation to a
  longer sibling.
- **A skill's frontmatter `description:` cannot hold a colon.** An unquoted `: `
  ...
  with no trigger text. Use a comma or a period.
- **The worktree isolation guard refuses any Bash command whose git use it cannot
  ...
  groups and shell arithmetic over `$(...)` all read as too complex. Use plain
  single commands and `python -c` one-liners, and write scratch inside the
  worktree or `$TEMP`.
...
- **`scripts/literal_defaults.py` prints `total N` to stderr and one site per line
  to stdout**, so `2>&1 | wc -l` reports N+1. Read the total from stderr.
- **A Windows esbuild run reproduces the committed bundle byte for byte.** Compare
  ...
  byte deltas on `calculator.js`, `calculator.css` and the LEGAL text are those
...
- **`scripts/extract_modules.py` check mode refuses a checked-in assignment
  record**: the split has been applied, so it reports "not a unit of" the source.
  The records are review evidence, never a replay input.
- **Deleting an installer deletes the last executable copy of its command.** When
  a doc becomes a command's last carrier, put its code blocks through the real
  argument parser in a test; `shlex.split` needs `block.replace("\\\n", " ")`
  first or the continuation survives as a token.
- **A regex written as ``` ``?NAME``? ``` requires one mandatory backtick plus an
  optional second, not an optional pair**, and matches only the double-backticked
  spelling. Use `{0,2}`, and print what a new scanner found.
- **The file-length hook measures against the committed file**, so any addition to
  a file already past the 500-line cap blocks the edit. Put a new pin in the small
  sibling that owns the idea. When the addition is a dict entry inside an existing
  assertion there is no sibling: take the hit, add no marker, and say so in the
  report.
- **Sizes from `git ls-tree -r -l` are MiB when divided by `2**20`.** Two reports
  disagreeing by about 5% on tracked bytes are agreeing in different units.
- **A rename's blast radius is not the files its diff touches.** Two defects in one
  unit were a `monkeypatch.setattr(module, "oldname", ...)` beside an attribute
  read in a file the diff never opened, so sweep `.<oldname>` and `"<oldname>"`
  over `src/`, `tests/` and `scripts/`. Dropping an alias's underscore can rebind
  it to another module's same name: `_coalesce_darius_q_heals` collided with the
  `survival` function the module imports plainly.
...
- **Two V8 deoptimizations live in `fingerprint`'s pixel loop.** An expando
  property on a typed array (`vec.contrast = rms`) deoptimizes every function that
  touches it, about 10x here, so return the number instead. `Math.min` and
  `Math.max` type their result float64, taking the typed-array index off the
  integer path, 2.8 s to 8 s, so the edge clamps there are ternaries. Time any
  change against `git show HEAD:static/js/scoreboard.js`.
- **Probe the page under its real CSP; `bypass_csp=True` hides the failures users
  ...
  bypassed probes pass. Decode a user's blob with `createImageBitmap`. Under the
...
- **The scoreboard reader is tuned at 24 to 36 px portraits (`PORTRAIT.min`).** A
  ...
  `PORTRAIT.typical`. A read that misses rows or items in one browser and not
  another is the input's scale: reproduce it by rescaling through
  `scoreboard_corpus.py read`, never by chasing color management.
...
- **`tests/js/harness_context.mjs` answers an unknown window key with a truthy
  ...
  throwaway and every later read gets a fresh one. Seed the namespace before
  running the script, and gate the page's real load order in the template test.
...
- **Known-degraded wiki parses, stable across patches.** The modifier parser
  ...
  for its mechanic anyway, so the parse fix belongs there.
- **`CachedSentence.match` searches each effect description separately and returns
  ...
  under `re.DOTALL`, without which the `.` cannot cross a newline and the marker
  stops guarding.
...
- **A cached `Max Health Damage` leveling row is often a percent-of-maximum-health
  ...
  for the shape. Read whose health the effect's prose pays before pricing it:
  Rek'Sai P, Tahm Kench E, Trundle P, Zac P and Dr. Mundo P each carry a heal
  under that name.
- **A threshold can exist only in the game files.** Dr. Mundo E's "0% to 40%
  ...
  file for a `Max...Threshold`; the wiki's "Maximum ..." row cross-checks it,
  equalling `minimum x max_amp`.
- **A form champion's stat deltas come from the game files, never the wiki stat
  ...
  Gnar P's cached `leveling` is empty. Verify every transform champion the same
  way: Gnar, Nidalee, Jayce, Elise, Shyvana.
- **Naafiri's binary names her two upper spell slots the opposite way round from
  ...
  swap runs through `data/atoms/v2/naafiri.atoms.v2.json` too.
- **A summarized fetch of a rendered champion page garbles progression tables and
  ...
  champion JSON and rules do not, so a fetched rule needs the game file or the
  user.
...
- **A `Bonus Damage` leveling row is often a monster-only cap, not a damage
  ...
  Rumble P carries the same shape. This engine's `target_class` has no monster
  value, so such a row is documented, never added.
...
- **Decorating a module-level slot keeps `<name>.phase = BUFF` working**, because
  ...
  `slot_entries.py` can never import such a decorator from `module_helpers`, which
...
- **A rotation declaration is read by key, so its own key order is free, but the
  ...
  so a hand-built row must take the same position.
...
- **When a form or steroid grants AD, decide base against bonus explicitly.** A
  ...
  %bonus-AD ratios see the grant and whether Sheen-type base-AD scalings grow.
...
- **A charge ability reports single-cast damage** and takes `rechargeRate` as its
  ...
  R): per-missile damage on `DamagePart.amount`, the count on `DamagePart.count`,
  and `cast_instances` set so per-cast item procs count each shot.
...
- **A granted or forced basic attack still swings when there is no auto stream.**
  One-rotation mode and zero uptime both give `num_auto_attacks == 0`, and the
  cast still forces its attack, so the ability's row carries the expected-crit
  base swing (Blitzcrank E, Vayne Q, Caitlyn headshots). Cap conversions by the
  auto count only when autos exist.
- **An empowered swing belongs on the ability's row in every fight mode.**
  ...
  `breakdown["auto_attacks"]["count"]` means plain attacks and anything needing
  the true attack count adds the empowering abilities' `casts x hits` back.
  On-hit, proc and stacking-DoT rows are deliberately not re-attributed.
- **An empowered attack that REPLACES the swing rides `auto_attack_conversion`,
  ...
  instead of magic resistance. The module supplies only the non-AD remainder,
...
- **Every per-ability side effect in the rotation loop is gated on the ability
  ...
  `num_casts == 0` with the entry still in `ability_damages`, and an ungated shred
...
- **Penetration floors at 0; resistance REDUCTION does not.** A flat shred (Corki
  ...
  already-negative resistance, because multiplying a negative by `(1 - pct)` gives
  resistance back. Route every `* (1 -` and `max(..., 0)` on a resistance through
  the shared helper, and regression-test monotonicity.
- **An ability that applies on-hit effects must apply ITEM on-hits**, and share
  ...
  key, which owns the closed On-Attacking list; the application declares
  `triggers=`.
- **Cap the triggering events, never the resolution.** A burn or DoT tail lit
  ...
  failure shape. The golden sweep's builds hold no burn items, so a timed-burn
  regression produces zero golden diffs.
- **A silent zero comes from a missing key.** A champion module never
  ...
  come from `stats.py`. A wiki unit spelling absent from `_SIMPLE_UNITS` in
  `champions/scaling.py` falls through to `0.0` and drops the term, as "% of
  maximum health" against "% maximum health" zeroed Rumble W: add the alias and
...
- **Never expose a per-ability override of a value another system derives.**
  ...
  describing the state, derive both consumers from it, and assert the invariant at
  every option value.
...
- **A row riding the auto stream must carry the prefix the split understands.**
  ...
  in the total. Assert a new row's bucket, not just its total.
- **An ability row carrying basic-attack swings sets
  `DamagePart(basic_damage=True)`** so both pricers apply `state.basic_amp` and
  accumulate `state.basic_amp_ability_bonus` (Caitlyn headshots against Hexoptics
  C44). Spell-classified rows stay unflagged.
- **Cast times share ONE timeline.** `damage.py::_schedule_shared_casts` runs
  ...
  `duration / (cast_time + cd)` and expect displacement. A total pinned under a
  superseded engine model embodies that model's defect.
