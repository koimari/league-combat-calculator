...
| SD9 | All five remaining tags retired (four number-bearing; `secondary_target` compiled nothing and its retirement closed a latent skip); flat readers + `magic_damage_amp` compiler; parity green per commit; claims layer re-pointed at the catalog. |
...
| SC3 | Sivir W prices the Bounce Damage row: 40.8 → 51.0 per bounce, and the published row now multiplies by the fight's bounce count (40.8 → 153.0 at three bounces). |
...
| ER2 | Aatrox Q/W re-timed with corpus+oracle re-pins; Annie/Kennen stack walks; Annie E priced; Vex R split; Jayce R rides `rides_scheduled_auto` (a missing forced swing surfaced and fixed); Aphelios weapons: three priced, Gravitum declared (a slow, damages nothing), Crescendum cache-blocked. |
