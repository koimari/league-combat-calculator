...
| Grey health (`timeline/grey_health.py`) | saturating accumulator `pool = min(cap, ratio·Σ post-mitigation incoming)` | **EXACT** | Out-of-vision consume (Pyke) is a vision boundary, documented not authored; cap = min(80 + 800% bAD, 55% max health) |
