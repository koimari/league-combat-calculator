# Scryglass onboarding, five minutes

For invited beta users. Everything you need to get a trustworthy answer from
Scryglass in five minutes. This is the companion to the first-run overlay in
`templates/index.html`, which is three steps and never blocks.
...
4. **Press “Find best item for a slot”.** That is it.
...
- **Rank, item icon and name**: what to buy, with its sourced stat line.
- **Score and metric**: the candidate's value under the selected objective
...
**Withheld · partial event order** rows are never ranked; treat the
...
- **Yes, No, or Off by %**: confirm the prediction, or report the damage you
