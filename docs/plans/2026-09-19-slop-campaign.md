...
| C | `d84b0771` | 4,736 removed against 1,572 added; `src/` 179,225 to 176,774 | 5 tracked, 3 added | Both units merged as one fast-forward chain, no conflicts. Full suite 16,430 passed, 32 skipped, against 16,584 collected on main and 16,462 here. Both goldens identical, and no baseline file moves against main, which is the union of the units' empty golden moves. Criteria: `src/` down 2,451 lines, the pylint gate names `E0102` in `tests.yml`, `ci/static.sh` and `CLAUDE.md`, and `import src.calculator.quantity` is 430 ms here against 574 ms on main, 389 modules against 547, short of the 100 ms criterion, which needs `champions/inputs.py` out of the champions package. One red the merge produced, fixed here: W8 dropped the underscore from `_action_key`, so `tests/test_transition_rank.py` resolved four call sites it had never read, and its population-wide one-level name binding read the Tahm Kench E local `rank` as a float in a phase slot; that local is `ability_rank` and no value moves. Two receipts the units left behind: `docs/receipts/er5-tail-triage.json` regenerated over the merged tree (696/140/388), and `repin_corpus` reverted as stamp-only. `capture_coverage_classification compare` prints identical, so the coverage receipt did not move, and every other regenerator is a no-op. The ruff tree gate reports 139 findings here against 144 on main, none of them new; that suite is at zero on neither. `TRAPS.md` holds 6,803 words against its 6,900 ceiling. One aside for the backlog: `max(1, ...)` leaves the Tahm Kench `if ability_rank == 0` fallback unreachable, which this branch does not touch because the fix moves numbers. |
| D1 | `82adcb67` | 3,606 removed against 3,589 added; champion tree 57,727 to 57,502 | 0 tracked, 2 added | Every gate ran on the merged tree; the only commits above the merge point are the traps append and this row, and neither touches `src/`. All five units merged, as one fast-forward chain over `main`, no conflicts: the chain already contained the Tahm Kench fix, so `git merge-base main <head>` is `main`'s tip. Full suite 16,441 passed, 36 skipped, against 16,428 collected on main and 16,477 here, the rise being `tests/test_ability_prose.py` and `tests/test_slot_guards.py`. Both goldens identical, and no baseline file moves against main, which is the union of the units' empty golden moves: A2's licensed move was not needed, because every converted reader still matches the current cache. Criteria: 60 `derive_self_healing` declarations take one `SelfHealCtx` and none takes six positionals; a runtime probe reworded the cached sentence behind six A2 readers and each raised its own declared message, including Taric Q's ceiling, whose reader had never matched this cache because it spelled the health share `his` where the cache writes `Taric's`. Cached-entry guard sites fall 171 to 59 (`ctx.ability` 129 to 25, `ctx.ranked` 42 to 34, counting a guarded call anywhere in a body), and literal-default row reads fall 72 to 14, of which `event.get` is 46 to 1. Every regenerator is a no-op: `behavior_frontier --write`, `cast_dependency_audit --output` and `item_umbrella_audit --output` produce no diff, `capture_coverage_classification compare` prints identical, and `repin_corpus` was not run because no data file moved, so no stamp needed reverting. Ruff falls 139 to 128, all drops in files these units touched. Two stale claims in `CLAUDE.md` corrected here after measuring both against main: `sightline gate . --full` reports 22 blocking above `.sightline-baseline` on main and on this branch, not zero, and the E-class census is three families, not two. `TRAPS.md` 6,803 words to 8,232, its budget raised to 9,700 by the test's own formula with Wave D1's 1,429 words as the widest wave measured. `CLAUDE.md` 1,349 words, `architecture.md` 7,056. `ci/container.sh` skips rather than passes: Docker is not installed on this machine. Two asides for the backlog, both named by units and re-measured at HEAD, where both unit figures were low: 8 prose sites in 5 files name two option keys neither `Rumble` nor `Seraphine` declares, and all 27 cross-slot `ctx.ranked("<slot>")` calls are guard sites wanting a `ranked_at(slot, index)` factory that `module_helpers.py` has no room for under the 500-line cap. |
