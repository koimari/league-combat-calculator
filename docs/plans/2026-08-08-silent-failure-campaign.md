...
| **H5** | **Scope the compiled-kernel extension** — timed, typed damage modifiers (D-101) | Multi-week and unbudgeted; it decides whether the compiled lane and the performance thesis are achievable at all | Phase 3 declares empty compiled coverage, every amp item falls back forever, and the performance goal is not met. **This cannot be defaulted — it is scoped or explicitly descoped here, in this table, and no phase document may write the descope into its own prose.** Both branches are already specified: if descoped, Phase 3 declares every `delta_amp` `ReceiptOnly` and Phase 4 asserts no compiled amp; if scoped, it lands as its own stage after Phase 4 S7 with its own equivalence fixture, and the flip from `ReceiptOnly` to `Compilable` is that stage's one-symbol commit under D-98 |
...
1. **The invariant holds by machine.** Every serialized numeric leaf of `/api/calculate` — score, breakdown,
   survival, TDD and timeline — **and of `/api/bis` (`app.py:1231`) and `/api/optimize` (`app.py:1266`), the
