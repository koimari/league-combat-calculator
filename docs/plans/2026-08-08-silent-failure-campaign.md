...
1. **The invariant holds by machine.** Every serialized numeric leaf of `/api/calculate` — score, breakdown,
   survival, TDD and timeline — **and of `/api/bis` (`app.py:1317`) and `/api/optimize` (`app.py:1390`), the
...
7. **Declarations outrank prose.** Phase 3's behaviour frontier reports counter 1 (runtime item-name
   ...
   > `raw_active, source.damage_type, resists, state.magic_amp` (`damage.py:14325`) — and
   > `damage._add_item_proc_damage` multiplies its mitigated per-proc figure by the holder's ability
   > amplifier — `amp = state.ability_amp if source.is_ability_damage else 1.0` (`damage.py:10931`);
   > `survival.pricing.price_declared_packet` (`pricing.py:339`) has neither. So
   ...
   > `survival.pricing.price_declared_packet` (`pricing.py:339`) prices a declaration at the **one**
   ...
   > (`damage.py:9894`) for physical packets, `_apply_liandry_reprice` (`damage.py:16321`) for magic
   ...
   > `_reattribute_empowered_swings` (`damage.py:17571`), which moves damage between two authored
   ...
   > what `survival.pricing.price_declared_packet` (`pricing.py:339`) carries. A Runaan's bolt is
   > priced by `_mitigate_basic_attack_swing` (`damage.py:901`), which applies three further
   > target-side terms, and the family's *other* authored row is the attack's on-hit effects copied
   > onto a second subject by `_copied_on_hit_packet` (`damage.py:15026`), for which
   ...
   > and the plating multiplier `_apply_target_basic_damage_reduction` (`damage.py:866`) applies —
   ...
   > `repricing_window_declarations` (`golden_snapshot.py:1433`) already reads the window mapping
