...
7. **Declarations outrank prose.** Phase 3's behaviour frontier reports counter 1 (runtime item-name
   ...
   > (`damage.py:9887`) for physical packets, `_apply_liandry_reprice` (`damage.py:9375`) for magic
   ...
   > `_reattribute_empowered_swings` (`damage.py:10210`), which moves damage between two authored
   > rows with the fight total untouched, and `_resolve_starting_shield_outcome`
   > (`damage.py:10579`), which re-prices every max-health-scaled packet against the target's live
   ...
   > `_apply_command_amp` (`damage.py:9832`) and `_apply_general_amplifiers` (`damage.py:9720`)
