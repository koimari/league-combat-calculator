...
7. **Declarations outrank prose.** Phase 3's behaviour frontier reports counter 1 (runtime item-name
   ...
   > `raw_active, source.damage_type, resists, state.magic_amp` (`damage.py:8211`) — and
   > `damage._add_item_proc_damage` multiplies its mitigated per-proc figure by the holder's ability
   > amplifier — `amp = state.ability_amp if source.is_ability_damage else 1.0` (`damage.py:7701`);
   ...
   > (`damage.py:9894`) for physical packets, `_apply_liandry_reprice` (`damage.py:9676`) for magic
   ...
   > `_reattribute_empowered_swings` (`damage.py:10518`), which moves damage between two authored
   ...
   > `_apply_command_amp` (`damage.py:10117`) and `_apply_general_amplifiers` (`damage.py:10028`)
