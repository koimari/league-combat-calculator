...
**R-04 — Gate verdicts reuse `scripts/gate_receipt.build_receipt`.** `scripts/validate_receipt.py` checks both the acceptance and champion-optimizer envelopes at `.github/workflows/tests.yml:17` (`validate_receipt.py`), and `tests/test_gate_receipt.py::test_ci_validates_every_receipt_it_emits` holds every receipt that step's matrices write to that step's arguments. A second receipt shape is a second thing that can quietly stop being true.
