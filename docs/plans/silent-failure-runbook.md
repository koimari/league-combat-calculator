...
**R-13 — Golden equality is equality to two decimals.** Verified: `_rounded` (`golden_snapshot.py:148-160`) rounds every float before write and `compare` recomputes with identical rounding. A summation-order change under 0.005 per leaf is invisible, so bit-exactness claims (Phase 4's `SumPlan`) need their own assertion and may not be discharged by golden. The instrument for those claims is `golden_snapshot.py capture-coupled --exact`, which writes `scripts/golden_coupled_exact.json` with `repr(float)` values, is excluded from the 2-dp compare, and is gated by its own equality test.
