...
**R-13 — Golden equality is equality to two decimals.** Verified: `_rounded` (`golden_snapshot.py:129-141`) rounds every float before write and `compare` recomputes with identical rounding. A summation-order change under 0.005 per leaf is invisible, so bit-exactness claims (Phase 4's `SumPlan`) need their own assertion and may not be discharged by golden. The instrument for those claims is `golden_snapshot.py capture-coupled --exact`, which writes `scripts/golden_coupled_exact.json` with `repr(float)` values, is excluded from the 2-dp compare, and is gated by its own equality test.

**R-14 — Provenance metadata is excluded from compare through one named constant.** `compare` today pops only `metadata.git_head` (`golden_snapshot.py:90-96`); the fingerprint block adds `src_tree_sha`, which changes on a comment-only edit. Both live in `COMPARE_EXCLUDED_PROVENANCE`, and a test asserts the numeric sections are not in it. Why: without this, "zero diffs on a pure refactor" becomes false for every commit and the strongest gate in the campaign gets routinely waived.
...
**R-29 — The one-walk property is asserted structurally, not measured.** Kernel invocations per pass, `SurvivalAction` construction sites, dispatch ladders in `survival/`, and "no view module performs arithmetic" are counted from source against the stage's expected value, **with the counting rule stated so the number is reproducible**. Measured today: one kernel with **two** invocations (`participant_timeline.py:2045` receipt, `:2809` score); **nine `SurvivalAction(...)` construction expressions** — measured on the pre-Phase-4 tree, where they were `participant_timeline.py:2764`, `survival/actions.py:203, :451` and six in `survival/compile.py`; eight of the nine moved into the one constructor at Phase 4 S4 and, re-measured at Phase 4 S10, are `program/compile.py:155, 392, 523, 638, 967, 1083, 1145, 1246, 1289, 1430` — ten rather than eight, because later stages added constructors to the one home and S10's row-reader unification removed one — leaving `survival/actions.py:601`'s default row as the declared survivor — a bare `grep -c "SurvivalAction("` returns 11 because it also matches the class statement and a docstring; **seven `survival_action_from_event` call sites** — on the same pre-Phase-4 tree, six in `participant_timeline.py` and one in `survival/receipt_state.py`; the builder is `program/compile.py`'s `action_from_event` since S4, its timeline call sites are `participant_timeline.py:2340, 2364, 2376, 2764` — four rather than six, re-measured at Phase 4 S10 after the stages that consolidated them — and the kernel's is the injected `compile_event` in `survival/receipt_state.py`; three compile scopes; one dead second ladder. Why source counts: a second engine regrows as code long before it shows up as a number.
...
| `scripts/investigator_brief.py` | the agent spawning a re-adjudication | Assembles a docketed dissent cluster's clause-1 brief: the export command quoted out of R-18 rather than re-spelled, the cluster's question and the defect its prior brief carried, and the **pre-change** series read out of the committed capture the docket names — never off the working tree, so the amendment's pre-change-only line holds structurally. It refuses a cluster routed to a ruling |
| `scripts/receipt_walk_schedule.py` + `docs/receipts/receipt-walk-retirement-schedule.json` | the lane sizing counter 4's receipt-walk debt | Each of counter 4's `(family, RECEIPT_WALK)` deferrals as the slice umbrella Amendment F says it is: the family's declarations and owners read from the catalog, the committed coupled scenarios covering them, and the qualifying population R-20's second half requires a slice to enumerate **before** its first `src/` edit. It retires nothing and registers no interpreter; its family set is asserted equal to the frontier's own deferral rows, so sizing the debt can never shrink it |
| `scripts/verify_backlog.py` + `docs/receipts/verify-backlog.json` | the lane preparing criterion 11's residue | One startable row per slice group carrying no recorded R-35 answer in the verify ledger: the commits it spans by subject, the plan documents its stated completion criteria live in, and R-35's brief **read from this document at run time** rather than copied. It records no answer and prepares every pass it lists; its row set is asserted equal to the ledger's own residue list, so preparation can never become redefinition |
