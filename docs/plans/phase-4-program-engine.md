...
5. **Public schema.** `CAPABILITY_SCHEMA_VERSION` is **4** at the phase tip — S9's value in D-63's chain,
   ...
   **every numeric leaf of the `/api/calculate` payload — score, breakdown, survival, tdd and timeline —
   ...
   mechanism.**
   **The same holds for every numeric leaf of the `/api/bis` (`app.py:1317`) and `/api/optimize`
   (`app.py:1390`) payloads, which the score view produces** — otherwise every criterion here passes while
