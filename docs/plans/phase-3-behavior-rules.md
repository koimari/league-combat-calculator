...
- **`magnitude` is a four-member union at the whole-total slot** (`Fixed | RampPerSecond | TargetBonusHealthScaled | RampPerStack`) — exactly the shapes the five `amp_fraction` closures the registry used to compile implement, now one branch each in `magnitude_fraction` (`interpreters/delta_amp.py`). This union *is* the removal of `DamageAmplifierEffect.amp_fraction: Callable`, landed with the whole-total chain slot in 3.2; the pre-migration citation into `item_effects` is deliberately not restated, because the referent it named is what the slice deleted. **A later chain slot may add a member and one branch with it, never a branch without a member**: Bloodsong's slice added `MeleeRangedSplit`, because 8% melee against 5% ranged is a magnitude conditioned on a build fact and `Typing.attack_classes` answers a different question — how a number was *delivered*, which is a property of the damage rather than of the champion holding the item. Closure is the test, not the arity.
...
- **Shadowflame Cinderbloom lands last, and the Liandry reprice is extracted in its own prior commit.** The max-health repricing of the Liandry burn key rode inside the ordered-ledger walk Cinderbloom's own function owned, and is an unrelated mechanic; carried along, the Shadowflame commit silently owns it and its diff is unattributable. The extraction commit gave it both halves of its own home — `_liandry_max_health_reprice` (`damage.py:2441`) computes the per-tick delta and `_apply_liandry_reprice` (`damage.py:10297`) folds it onto the burn's own row — and renamed the shared walk `_simulate_ordered_damage` (`damage.py:2470`), which is what it always was. Shadowflame's authority move is Phase 4's, last of seven.
...
- **A frozen declaration may hold references, never numbers.** `Const` is structural only — counts, caps, ranks, booleans — and a reflective test asserts every non-integral float field is a `ValueRef` while every raw number a declaration holds carries a closed `StructuralReason` (`Const.__post_init__` refuses a non-integral value for any reason but `unit_scale`). `ValueRef.get()` routes through `required_effect_value` / `ally_item_effect_value` / the rune accessor, so rule 5's fail-loud behaviour survives with item-and-key context.
...
- **`item_coverage.item_model_coverage` stops branching on names and computes status from declarations.** `withheld` (a declared family has no interpreter in a needed lane) and `review_pending` (described effects, no declaration) become automatic; the ten hand registries collapse to two — the reviewed `NO_RUNTIME_BEHAVIOR` prose the UI needs, and `_REVIEW_ISSUE_REFS`. Three of the ten (`_BLOCKED_REASONS`, `_CALCULATION_ALLOWED_BLOCKED`, `_PARTIAL_BLOCKED_REASONS`) were already empty when this document was written — two dicts and one bare `frozenset`, so the assertion is emptiness, never `== {}`; 3.8's first commit pins that emptiness and its second deletes all three, which is why the line citation this sentence used to carry is gone rather than refreshed — and are asserted empty before deletion, so the honest count is seven real registries plus three empties. **Its signature and return type both change** — `item_model_coverage(name: str, needed: frozenset[EngineLane]) -> ItemCoverage` — so every caller moves in the same commit: `src/app.py:1033, :1081` (which serialize it straight into the public payload), `item_coverage.py:609, 689, 712, 717, 739`, `participant_timeline.py:700`, and `tests/test_item_coverage.py`. `src/app.py` is L4's for this flip, and because the serialized coverage payload changes, **3.8 bumps `CAPABILITY_SCHEMA_VERSION` to 3** — the third value of D-63's chain, after 0A's 1 and C4's 2. Phase 4's S9 owns the next bump; this document states no value but its own.
...
- **Counters 2 and 4 are read under the umbrella's dated amendments A and B** ([umbrella](2026-08-08-silent-failure-campaign.md), criterion 7): counter 2's Class C exclusion set gains Phase 1's authored claim-evidence corpus containers and `_REVIEW_ISSUE_REFS`, and counter 4's `RECEIPT_WALK` gaps that only Phase 4 S3 can retire become committed, diff-gated deferral rows the exit target is measured net of. This line is a pointer, not a descope — the ruling and its arithmetic are the umbrella's.
...
class BehaviorRule(family, owner, mechanic_id, payload, compilability, receipt, zero_policy)
    ...
    #  champions/slotlib.py, overridable per call, so the 377 champion call sites
...
7. Every non-integral float in every declaration is a `ValueRef`; every raw number a declaration does hold is structural and says which kind it is — `Const`'s closed `StructuralReason`, refused at construction for anything non-integral, plus the validated `lane_chain_rank` ordering indices (the retired spelling of this clause named `_STRUCTURAL_EFFECT_KEYS`, which is the item registry's code-owned-key set and governs registry values rather than declaration fields; the *Values, receipts and refresh* ruling and the Shape block's `Const(value, reason: StructuralReason)` are the authority, and this criterion now agrees with them). Every rule carries a `zero_policy` naming one of the four dispositions, and every `damage_entry`/`simple_damage` carries one through **D-24's ruled exception** — the single declared default (`MEASURED`) at the one construction layer, `champions/slotlib.py` (`:432`, `:652`), overridable per call, so the 397 call sites across 151 champion modules are deliberately **not** edited and no champion sweep is implied; a test asserts the default lives only at that layer. **The exception's guard ships with it**: a source assertion over `champions/` forbids a `.get(key, <literal>)`-shaped fallback from feeding a damage formula — rule 5's no-stale-literal discipline extended to champion inputs — so a zero produced by an unwired stack count or option fails loud instead of being stamped `MEASURED` by the default. The counting rule is the receiver, not a dataflow: the three champion **input** blocks (the build's stats, the target's stats, the user's options) exist for the formulas, so the shape is refused on any of them anywhere under `champions/`, while the same shape on a block the tree *produced* is ratcheted instead. `champions/inputs.py` holds the vocabularies and declared defaults that replaced them, and every declared name is asserted against the producer that emits it. The `zero-policy-frontier` set for leaves produced by neither is committed, issue-reffed and asserted non-growing.
