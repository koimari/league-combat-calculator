...
# ``(live_generation(...), id(item_data))``.  Each entry keeps a strong
...
# ``live_generation`` rather than ``data_version`` because this memo has no
# size bound: prefixing the key made a superseded entry unreachable but not
# collected, so the generation that supersedes it drops it.
...
def _validate_cached_item_stats(item_data: dict[str, Any]) -> None:
    ...
    memo_key = (live_generation(_ITEM_STATS_VALIDATION_MEMO), id(item_data))
...
def get_item_stats(item_data: dict[str, Any]) -> dict[str, float]:
    ...
    memo_key = (live_generation(_ITEM_STATS_MEMO), id(item_data))
