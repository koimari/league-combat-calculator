...
- W (Yordle Snap Trap) deals no damage itself, but the generic
  ...
  standalone magic nuke. The slot is deliberately absent; W's only
  modeled effect is the increase on P's single trap headshot.
...
def _trap_headshot_increase(ctx: SlotCtx) -> float | None:
    """W's flat damage increase on the one trap headshot; None = no trap.
...
def _headshot(ctx: SlotCtx) -> dict[str, Any] | None:
    ...
    trap_used, e_used, cadence_used, swings, detail = _headshot_counts(
        ctx, 0 if trap_increase is None else 1
    )
