"""Exact packet wrappers for the CP10.10 full-entry Wiki boundary."""
...
_SOURCE_PATH = (
    Path(__file__).resolve().parents[3] / "static" / "cp10_batch_10_sources.json"
)
...
def _full_entry_sources(name: str) -> list[dict[str, object]]:
    ...
    try:
        ...
        raise RuntimeError(
            f"CP10.10 source receipts are unavailable: {_SOURCE_PATH}"
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"CP10.10 source receipts for {name!r} are incomplete")
    return rows
...
def build_batch_module(name: str):
    ...
    if name not in BATCH_10:
        raise KeyError(f"CP10.10 does not contain {name!r}")
