"""Exact packet wrappers for the CP10.11 full-entry Wiki boundary."""
...
_SOURCE_PATH = (
    Path(__file__).resolve().parents[3] / "static" / "cp10_batch_11_sources.json"
)
...
def _full_entry_sources(name: str) -> list[dict[str, object]]:
    ...
    try:
        ...
        raise RuntimeError(
            f"CP10.11 source receipts are unavailable: {_SOURCE_PATH}"
        ) from exc
    ...
    if (
        ...
        raise RuntimeError(f"CP10.11 source receipts for {name!r} are incomplete")
    return rows
...
def build_batch_module(name: str):
    ...
    if name not in BATCH_11:
        raise KeyError(f"CP10.11 does not contain {name!r}")
