...
E (Eye of the Storm) shields the selected teammate and R (Monsoon) heals the
caster and all allies.  Both events are authored by the engine's ally-support
scanner from cached leveling at the cast times, and E and R are declared in
SLOTS so the fight rotation casts them.  R's heal is delivered as the sourced
Total Heal at cast time; the cached per-tick row is the cadence detail, twelve
ticks summing to that total.
"""
