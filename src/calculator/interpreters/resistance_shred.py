...
class ResistanceShredPairInterpreter:  # pylint: disable=too-few-public-methods
    ...
    def compile(self, rule: BehaviorRule, ctx: BuildContext) -> tuple[KernelField, ...]:
        """The three sourced numbers a shred's ramp resolves to.

        The *model* is not a field: it is a policy the slot branches on, and
        compiling it to a value would let a caller read a summation rule off a
        number.
        """
...
def resolve_slot(
    ...
    *,
    ...
    """This build's shred of one resistance, or ``None`` if nobody declares one.
