...
- **Root cause:** R passive grants armor pen per rank, which should always be active, and the pre-engine module priced Q/W/E before applying it.
- **Pattern to watch for:** Any ability with a passive stat component is a BUFF-phase `stat_buff` slot (with `apply_to=` when the stat scales other abilities at parse time), so the engine guarantees damage slots see it.
...
- **Root cause:** The engine had no per-item trigger classification. The wiki's On-Attacking list is short and closed (Guinsoo's Rageblade, Navori Flickerblade, Rapid Firecannon, Runaan's Hurricane, Voltaic Cyclosword, Yun Tal Wildarrows, which trigger on completing an attack windup); everything else per-hit is on-hit. Each item declares its class as the registry's `counter_trigger` key (`item_effects.counter_trigger(name)` reads it, `"on_hit"` by default); ability applications declare `triggers: ("on_hit",)` or `("on_hit","on_attack")`.
...
### Akshan (latent, found during Bel'Veth work): E attack-speed scaling reads 0
- **What happened:** E's attack-speed term priced as 0 with every real build: the stat block carries `bonus_attack_speed`, and the module read a `bonus_attack_speed_percent` key nothing writes. Still zero by declaration: `akshan.py` states the 0 explicitly under the escalated-defect receipt (`docs/receipts/escalated-defects-P3-3.7.json`) because correcting it moves his golden numbers.
