    ...
    def _render_modifiers(self, mods: str, nvalues: int) -> List[Modifier]:
        ...
        for lvling in parsed_modifiers:
            try:
                ...
                print("ERROR:", error)
                fallback_n = nvalues if nvalues is not None else 1
                modifier = Modifier(
                    values=[0 for _ in range(fallback_n)],
                    units=[lvling for _ in range(fallback_n)],
                )
                modifiers.append(modifier)
        return modifiers
...
class ParsingAndRegex:
    ...
    def get_modifier(mod: str, nvalues: int) -> Tuple[List[str], List[Union[int, float]]]:
        ...
        if mod in list(LolWikiDataHandler.UNHANDLED_MODIFIERS.keys()):
            ...
            fallback_n = nvalues if nvalues is not None else 1
            parsed = [value for _ in range(fallback_n)]
            units = [lvling for _ in range(fallback_n)]
            return units, parsed
        else:
