...
- `champions/healing_contract.py` defines the typed self-healing declaration owned by a champion module. Each champion module owns its resolver and formula. `healing_helpers.py` owns shared lookup and event-construction helpers. `healing.py` loads declarations and owns generic healing application and receipt ordering. `champions/healing_rules.py` remains a retired compatibility surface.
- `roster_composition.py` owns the `Combatant` value object, loadout conversion, main and actor parameter construction, target override signatures, roster window validation, and Catalyst resource inputs. `participant_timeline.py` keeps the event enrichment and top-level orchestration that consumes those inputs.
