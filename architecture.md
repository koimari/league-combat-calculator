...
- `champions/healing_contract.py` defines the typed self-healing declaration owned by a champion module and the `SelfHealCtx` record every resolver reads, and each champion module owns its `derive_self_healing` resolver and formula. The declaration owns the receipt order, applied once in `self_healing_rule`. `healing_helpers.py` owns what those resolvers share: the sourced readers and the payment anchors. What a rule pays on (a cast, a hit that dealt damage, or a tick schedule of its own) is declared by the rule (`HealAnchor`) and never inferred from how many events a module authored. `healing.py` loads declarations, derives `HEALING_RULE_CHAMPIONS` from the modules that export one, builds the `SelfHealCtx` the pipeline hands in, and owns generic healing application. Champion-option state a heal needs (`self_heal_state`, `self_heal_share_of_damage`) is priced in the module entry, where both the options and the target are in hand.
...
- `item_support_effects` is the dispatch: four tables keyed by `AllyProducer`, the walks over them, and the two entry points `participant_timeline` calls. One block per declared producer lives in `item_support_quests`, `item_support_everlasting`, `item_support_shred`, `item_support_triggered` and `item_support_actives`, the fight inputs every block reads are `support_context`, and `_validate_producer_tables` refuses at import a producer with no block or with two. A capability's `trigger_stream` `impl` names the block that builds its packet, and that is the module a coverage claim's packet literal resolves against. The residue loads 12 internal modules against sightline #27's fan-out ceiling of 10, and only its reader count keeps that arm quiet: the arm needs two internal readers and `participant_timeline` is the one. The engine reaches the module's facts through `AllyPacketSlot.control_arming` in `interpreters/ally_packet.py` and through `survival.actions.event_timestamp`, never by importing it. A second reader means baselining the arm beside the price arm with that reason.
...
| `attack_cadence.py` | when a basic attack lands: T=0 is the attack command, impact `k` lands once the timer has run `k + phase` cycles, and the champion's windup sets `phase`. Every swing stream and every champion module that mirrors one places its impacts through it |
...
Cast *order* has two surfaces and they are deliberately separate. `cast_dependency.py` owns the declared vocabulary: what a champion module may assert about its own kit ("Scatter the Weak stuns only through a Dark Sphere her Q put on the field", so E requires Q), its four `DEPENDENCY_KINDS`, and the import-time validators the module contract runs. It also spells the twelve `INFERRED_EDGE_KINDS` the resolver may conclude. Inference is the resolver's: `cast_edge_inference.py` concludes those kinds from parsed markers, `cast_edge_resolution.py` merges declarations over the inferences, and `rotation_resolver.py` derives the cast order from the merged edges. The two vocabularies are closed and disjoint so an inferred classification can never be mistaken for a module's assertion. `cast_dependency.py` is a stdlib-only leaf: it imports nothing from `src/calculator`, which is what lets the resolver, the champion package and the pipeline share one vocabulary without the champion package owning the resolver's taxonomy. `champions/__init__.get_champion_cast_dependencies` is the one accessor; the validated contract is its only source.
...
`participant_timeline.py` composes the roster, per-pair event ledgers, and one coupled survival walk, and owns `CoupledSearchContext`, the per-search compile-once caches. `timeline/` holds the records that composition writes through and the steps that read only those: `records.py` (`TimelineScene`, `Roster`, `Ledgers`, `GreySubject`, `Walked`, `PairCacheKey`), `grey_rates.py` and `grey_health.py` (the pool banked from the packets that landed and repaid by the champion's active), `utility.py` (one actor's utility outcome receipt) and `receipt.py` (the published combat receipt, its sorted ledgers and its objective fold). `program/` is the logical layer above the kernel, "what happened, and to whom": `events.py` closes what a rider may be, the five `RIDER_KINDS` an event carries and dies with, `route.py` resolves each event's subject once and totally, `build.py` names the roster one composition pass walks, with `ParamPatch` the only declared difference between two passes, `compile.py` holds `WalkCompiler` and every action record's construction, and `walk.py` is the one `run_survival_walk` call site, returning a frozen `WalkResult`. `program/views/` holds its five projections (`receipt`, `score`, `survival`, `breakdown`, `tdd`); a view re-runs no arithmetic, every number it emits is already a leaf of the result, and takes every digit count from `program/precision.py`, which is where rounding lives. `survival/` is the transition kernel underneath: `typed_action` (`SurvivalAction`, every field the walk reads at its neutral value), `action_families` (the eight records the 21 kinds build, each storing the core plus the fields its kinds' walk stages read and reading every other field's neutral off `SurvivalAction`), `actions`, `transitions` (the walk and its dispatch ladder), `receipt_state`, `score_state`, `compile`, `pricing`, `accumulate`. The dependency runs `program -> survival` and never back, so the hot loop never dispatches on a logical type. The two packages re-export under opposite rules on purpose: `survival/__init__.py` is an API boundary, so its `__all__` is the declared kernel vocabulary an item or schema author writes against and growing it is an API change, while `program/` is one concept per module, so it re-exports nothing and every reader names the dotted path the derived front-door registry measures. `run_survival_walk` mutates the `actions` list its caller passes and re-reads `len(actions)` every iteration, because three producers insert into it mid-walk after `ledger.current_index`: that is how an action is rescheduled, a `NamedTuple._replace` at a new time keeping the `aidx` and event dict so the ledger records one outcome. A caller passes a real list and reads it back, since a copy drops every walk-authored packet silently.
...
- `tests/test_program_structure.py` pins the layering: one `run_survival_walk` call site in `src/`, no action record built outside `program/compile.py`, the one-way `program -> survival` import direction, view purity (no view nor anything it calls does arithmetic), and the allocation budget.
