...
- `champions/healing_contract.py` defines the typed self-healing declaration owned by a champion module. `healing.py` loads those declarations and owns receipt ordering. `champions/healing_rules.py` is the migration boundary for reviewed formulas. What a rule pays on — a cast, a hit that dealt damage, or a tick schedule of its own — is declared by the rule (`healing_legacy.HealAnchor`) and resolved by `healing_legacy._payments`; it is never inferred from how many events a module authored for the ability. A migrated rule can provide its resolver from `champions/<name>.py`; the compatibility resolver preserves old event output while the remaining rules move in focused slices.
